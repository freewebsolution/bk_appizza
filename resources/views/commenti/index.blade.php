                                ...
                                @foreach($commenti as $commento)
                                    <li class="list-group-item d-flex justify-content-between align-items-start">
                                        <div class="ms-2 me-auto">
                                            <div class="fw-bold text-muted">
                                                ...
                                                @if($commento->insalatona)
                                                    <i class="fa-solid fa-seedling"></i>  {{$commento->insalatona->titolo ?? '' }}
                                        ...
                                        @if(Auth::user()->name === $commento->user->name )
                                            <a href="{{action([\App\Http\Controllers\CommentiController::class,'edit'],$commento->id)}}" class="me-sm-2">Edit
                                                <i class="fa-solid fa-pencil"></i>
                                            </a>
                                            <form action="{{action([\App\Http\Controllers\CommentiController::class,'destroy'],$commento->id)}}" method="get">
                                                @csrf
                                                @method('DELETE')
                                                <button onclick="return confirm('Sei sicuro di voler eliminare il commento id: {{$commento->id}} ?')" class="btn btn-danger btn-sm" type="submit">
